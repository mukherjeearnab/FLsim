...
class CIFAR10Strategy(TorchStrategyBase):
    ...
    def __init__(self, hyperparams: dict, dataset_params: dict, is_local: bool, device='cpu', base64_state=None):
        ...
        if base64_state is None:
            ...
            # hyperparameters
            if self.is_local:
                self.temperature = self.client_extra_params['moon']['temp']
                self.mu = self.client_extra_params['moon']['mu']
    ...
    def train(self) -> None:
        ...
        self.prev_local_model = self.prev_local_model.to(self.device)
        ...
        for epoch in range(self.train_epochs):
            ...
            for i, (inputs, labels) in enumerate(self._train_set, 1):
                ...
                optimizer.zero_grad()
                proj_l, pred_l = self.local_model.forward_with_projection(
                    inputs)
                proj_g, _ = self.global_model.forward_with_projection(inputs)
                proj_pl, _ = self.prev_local_model.forward_with_projection(
                    inputs)
                ...
                contrastive_loss = self.mu * criterion(logits, mask)

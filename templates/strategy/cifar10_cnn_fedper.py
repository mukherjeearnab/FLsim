...
class CIFAR10Strategy(TorchStrategyBase):
    ...
    def parameter_mixing(self) -> None:
        ...
        with torch.no_grad():
            ...
            extractor_weights = dict()

            # remove the weights and biases of the all other layers
            # except the feature extractor
            for key in global_weights.keys():
                # feature_extractor is the name of the extractor network
                if 'feature_extractor' in key:
                    extractor_weights[key] = global_weights[key]
                else:
                    extractor_weights[key] = local_weights[key]

            print(extractor_weights.keys())

            # set the parameters for local as global model
            self.local_model.load_state_dict(extractor_weights)
